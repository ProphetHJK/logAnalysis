...
start_time = datetime_timestamp('2020-05-08 05:00:00')  #运行程序的当地时间
end_time = datetime_timestamp('2020-05-09 05:00:00')  #运行程序的当地时间
start_EOB_time = datetime_timestamp('2020-05-01 05:00:00')
end_EOB_time = datetime_timestamp('2020-05-09 05:00:00')
time_type = 'data_time'
EOB_month = 5

root_path = r'G:\沙特现场问题'  # 日志父路径
dir_name = '0510' #日志文件夹名
out_dir = root_path+ '\\output\\'
# out_dir = os.path.join(path, dir_name)+ '\\output\\'
if not os.path.exists(out_dir):
    os.makedirs(out_dir)

auto_search = 1  #自动查找path路径下日志文件夹
change_name = 0  #改名
...
if auto_search == 1:
    ...
    for sub_dir in dir_list:
        ...
        profile_name = sub_dir+'_'+time_type+'_'+str(start_time)+'-'+str(end_time)+'.csv'  # 数据库导出文件名
        srwf_name = sub_dir+'_3762.log'   # 3762报文文件名
        out_dir = root_path+ '\\output\\'
        # 第一部分：日志处理
